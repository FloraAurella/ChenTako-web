...
Worker 请求上限 20 MiB（含 JSON 编码），超过时返回 413；全文上下文不截断。云端上游仅支持标准端口 HTTPS，跳过公网域名 DNS 预检，但仍拒绝明显内网／本机地址、链路本地、私有 IPv6、映射 IPv4，以及重定向。Worker 使用手动重定向模式并显式拒绝 3xx，以兼容边缘运行时。四种协议与稳定应用 SSE 事件继续复用原后端；正文、摘要、标题仅使用 system/summary/title 文件。

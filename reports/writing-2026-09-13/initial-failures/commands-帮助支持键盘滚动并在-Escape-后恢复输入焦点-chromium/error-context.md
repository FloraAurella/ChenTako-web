...
- Name: commands.spec.js >> 帮助支持键盘滚动并在 Escape 后恢复输入焦点
- Location: e2e/commands.spec.js:289:1
...
- generic [ref=e4]:
  ...
  - generic [ref=e7] [cursor=pointer]: 今天
  - main [ref=e9]:
    - generic [ref=e10]:
      - generic [ref=e11]:
        - button "展开侧栏" [ref=e12] [cursor=pointer]
        - generic [ref=e16]:
          - generic [ref=e17]: Chat · 对话
          - generic [ref=e18]: 新对话
      - generic "检查中" [ref=e21]
    - region "对话" [ref=e25]:
      - generic [ref=e28]:
        - heading "欢迎回来，随时开始吧" [level=2] [ref=e35]
        - paragraph [ref=e36]: 正在检查连接…
      - generic [ref=e38]:
        - button "当前项目：无项目" [ref=e40] [cursor=pointer]:
          - generic [ref=e44]: 无项目
        - generic [ref=e48]:
          - textbox "输入消息，/ 打开指令" [active] [ref=e50]
          - generic [ref=e51]:
            - button "添加附件" [ref=e52] [cursor=pointer]
            - generic [ref=e55]:
              - button "model-a 中等" [ref=e56] [cursor=pointer]:
                - generic [ref=e57]: model-a
                - generic [ref=e58]: 中等
              - button "压缩上下文（已用约 0%）" [ref=e62] [cursor=pointer]
              - button "发送" [disabled] [ref=e67]
        - generic [ref=e70]: Enter 发送 · Shift + Enter 换行
  - navigation "主导航" [ref=e71]:
    - button "对话" [ref=e72] [cursor=pointer]
...
> 292 |   expect(await page.locator('.command-panel-host').evaluate(el => el.scrollTop)).toBeGreaterThan(0);
  ...
  376 |     const panel = page.locator('.command-panel');
  377 |     await expect(panel).toBeVisible();
  378 |     await expect(panel).toHaveAttribute('data-material', 'opaque');
  379 |     const paper = await page.evaluate(() => {
  380 |       const probe = document.createElement('div');
  381 |       probe.style.background = 'var(--surface-content)'; document.body.append(probe);
  382 |       const color = getComputedStyle(probe).backgroundColor; probe.remove(); return color;
  383 |     });
  384 |     await expect(panel).toHaveCSS('background-color', paper);
  385 |     expect(paper).toMatch(/^rgb\(/);
  386 |     await panel.hover();
  387 |     await expect(panel).toHaveCSS('background-color', paper);
  388 |     const selectedId = await page.locator('.command-option[aria-selected="true"]').getAttribute('id');
  389 |     const nextIndex = (Number(selectedId.split('-').pop()) + 1) % await page.locator('.command-option').count();
  390 |     await input.press('ArrowDown');
  391 |     await expect(page.locator(`#command-option-${nextIndex}`)).toHaveAttribute('aria-selected', 'true');
  392 |     mkdirSync('reports/command-opaque-2026-09-13', { recursive: true });
```
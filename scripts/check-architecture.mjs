...
for(const filename of walk(path.join(root,'src')).filter(f=>/\.[jt]sx?$/.test(f))){
 ...
 function inspect(node){
  if((ts.isImportDeclaration(node)||ts.isExportDeclaration(node))&&node.moduleSpecifier&&ts.isStringLiteral(node.moduleSpecifier)){
   ...
   const fileSpec=spec.replace(/[?#].*$/,"");
   const target=path.relative(root,path.resolve(path.dirname(filename),fileSpec));const other=target.match(/^src\/modules\/([^/]+)\//)?.[1];
   ...
   if(!['','.js','.ts','.tsx','.json','.svg'].some(ext=>fs.existsSync(path.resolve(path.dirname(filename),fileSpec+ext)))) failures.push(`${relative}: missing ${spec}`);
  }

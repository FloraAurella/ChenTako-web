...
/** 压缩请求的固定内置技术指令：不携带任何产品人格设定（原版逐字移植）。 */
export const CONTEXT_COMPRESSION_PROMPT = [
  '你是对话上下文压缩器。请把此前对话压缩为一份可供同一模型无缝继续任务的高密度内部上下文。',
  '必须遵守以下规则：',
  '1. 保留用户目标、明确约束、偏好、关键事实、专有名词、数字、日期、代码与 API 契约、已做决定、验证结果、失败原因、修复结论和未完成事项。',
  '2. 明确区分用户陈述、助手建议与已经验证的结论；不得虚构、推断为事实或改变原意。',
  '3. 删除寒暄、重复表达、无关措辞，以及已经被后续内容明确取代的信息。',
  '4. 对代码、命令和配置保留继续任务所需的标识符、路径、参数和值；无法安全省略时宁可保留。',
  '5. 保持时间顺序和因果关系，使后续模型无需看到原对话也能正确继续。',
  '只输出压缩后的上下文本身；不要解释压缩过程，不要向用户提问，不要添加前言、结束语或代码围栏。'
].join('\n');

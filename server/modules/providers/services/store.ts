...
/**
 * 供应商注册表 + API Key 保险库的本地存储。
 * - providers.sqlite（node:sqlite）：注册表记录，按插入顺序返回；
 * - API Key：AES-256-GCM 加密后入库；主密钥在数据目录 .master-key（0600），
 *   或由 CLAWBOX_MASTER_KEY（hex）注入；环境变量 CLAWBOX_API_KEY_<keyEnv> 为兜底来源。
 * 会话/聊天数据绝不落在这里——providers 与密钥是唯一服务端持久化数据。
 */
...
export class ProviderStore {
  ...
  constructor(dataDir: string) {
    ...
    this.db = new DatabaseSync(join(dataDir, 'providers.sqlite'));
    this.db.exec(`
    ...
    `);
    this.master = this.loadMasterKey(dataDir);
  }

import { module as writing } from '../modules/writing/public/module';
import { module as knowledge } from '../modules/knowledge/public/module';
...
export const builtins: readonly FrontendModule[] = [appearance,connections,context,extensions,data,projects,knowledge,commands,chat,writing,settings];
// ChenTako-web temporarily runs as a chat-only product; retain writing data and source.
const suspendedModules = ['writing'];
...
export function createComposition(disabled: readonly string[] = []) {
 ...
 const contributions: FrontendContributions = { requestContexts:new Registry(),commands:new Registry(),settings:new Registry(),domains:new Registry(),pages:new Registry(),controllers:new Registry(),slots:new Registry() };
 const runtime = createModuleRuntime(builtins.filter(m => !disabled.includes(m.id) && !suspendedModules.includes(m.id)), contributions);
 runtime.start();

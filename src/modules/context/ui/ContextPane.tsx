...
export function ContextPane({ store, service, projectId = "" }: { store: ExternalStore & { state: any }; service: SettingsService; projectId?: string }) {
  ...
  return <div className="settings-pane context-pane">
    ...
    <SettingsGroup id="context-common" title="常用配置" description="先确定助手指令与上下文策略。项目未覆盖的字段持续跟随聊天默认配置。">
    ...
    <SettingsGroup id="context-advanced" title="高级配置" description="按模型需要调整生成、输出和兼容性参数；无需修改时可保留默认值。">

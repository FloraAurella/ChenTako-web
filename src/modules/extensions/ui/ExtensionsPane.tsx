...
const meta: Record<Kind, { title: string; card: string; lede: string; desc: string; icon: string; key: string; action: string }> = {
  tool: { title: "工具", card: "自定义工具", lede: "管理模型可调用的 HTTP 工具。", desc: "设置工具地址、输入 Schema 和启用状态。", icon: "puzzle", key: "tools", action: "新增工具" },
  skill: { title: "技能", card: "技能", lede: "导入并启用可复用的模型指令。", desc: "支持 Markdown（≤16KB）和 Skill 压缩包（≤5MB）。", icon: "spark", key: "skills", action: "导入技能" }
};
...
function RuntimeSetting({ interpreter, checked, service }: { interpreter?: boolean; checked: boolean; service: SettingsService }) {
  ...
  const text = interpreter ? "在无网络 Linux 虚拟机中运行 Python，并用于后续对话。" : "在无网络临时工作目录运行受控命令，并用于后续对话。";

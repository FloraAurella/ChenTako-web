...
export function SettingsIndex({ section }: { section: string }) {
  ...
  return <aside className="settings-index" id="settingsIndex" aria-label="设置目录"><h2 className="settings-index-title">偏好设置</h2><label className="settings-search"><TrustedIcon name="search" size={16} /><TextField className="ui-input--quiet-focus" type="search" aria-label="搜索设置" placeholder="搜索设置…" value={query} onChange={(e) => setQuery(e.target.value)} /></label>
